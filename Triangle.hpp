...
// 光线与三角形的相交测试 Moller Trumbore Algorithm
...
public:
    ...
    // 当三角形作为光源时，需要在其表面采样点计算对场景中其他点的光照贡献
    // 在路径追踪中用于生成从交点出发到光源的采样光线
...
public:
    MeshTriangle(const std::string& filename, Material *mt = new Material())
    {
        ...
        bvh = new BVHAccel(ptrs);   // 构建BVH包围盒
    }

    bool intersect(const Ray& ray) { return true; }

    // 遍历所有三角形来判断是否有交点，tnear初始值为infinity
    ...
    // 根据给定的纹理坐标生成棋盘格图案的漫反射颜色，输入参数为纹理坐标st
    ...
    // 随机采样光源，但是采样的不一定是光源，若是其它物体，其emit=0
    void Sample(Intersection &pos, float &pdf){
        bvh->Sample(pos, pdf);
...
inline Intersection Triangle::getIntersection(Ray ray)
{
    ...
    if(t_tmp>0 && u>0 && v>0 && 1-u-v>0) {
        ...
        inter.obj = this;   // 这里的this指针指向的是当前三角形对象
        inter.m = m;
    }
    return inter;
}
